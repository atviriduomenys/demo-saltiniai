...
class County(AdministrativeUnit):
    class Meta:
        verbose_name = "County"
    ...
    def generate_test_data(cls, quantity: int = 1, **kwargs) -> list["AdministrativeUnit"]:
        counties = []
        for _ in range(quantity):
            settlement = Settlement.generate_test_data(quantity=1)[0]
...
class Municipality(AdministrativeUnit):
...
class Eldership(AdministrativeUnit):
    ...
    def generate_test_data(cls, quantity: int = 1, **kwargs) -> list["AdministrativeUnit"]:
        ...
        for _ in range(quantity):
            ...
            eldership = make_recipe(
                ...
                centre=municipality.centre,
                country=municipality.centre.country,
                _fill_optional=True,
            )
