...
def build_address_registry_nested() -> dict:
    ...
    municipalities = Municipality.objects.all().select_related("country")
    elderships = Eldership.objects.all().select_related("country")

    settlements_dict = [_get_settlement_dict(settlement) for settlement in settlements]
    counties_dict = [
        {
            **_get_administrative_unit_dict(county, settlements_dict),
        }
        for county in counties
    ]
    municipalities_dict = [
        {
            **_get_administrative_unit_dict(municipality, settlements_dict),
    ...
    elderships_dict = [
        {
            **_get_administrative_unit_dict(eldership, settlements_dict),
